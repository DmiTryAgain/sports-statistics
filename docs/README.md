...
│   │   ├── ratelimit.go          # Per-user rate limiter с фоновой очисткой
...
ReplyKeyboard "Показать"
  ...
  Сегодня / Вчера / Неделя / Прошлая неделя / Месяц / Прошлый месяц / Год / Прошлый год / Всё время
        │
        ▼
  Таблица статистики
```
...
Чистая функция `nextUnfilledParam()` определяет следующий незаполненный параметр, проходя три уровня:
1. **Required** — обязательные параметры категории (`RequiredParams`). Без них сохранение невозможно.
2. **Soft-required** — параметры, хотя бы один из которых обязателен (`SoftRequiredParams`). Остальные показываются с кнопкой «Пропустить».
3. **Optional** — опциональные параметры упражнения (`Exercise.OptionalParams`). Показываются с кнопкой «Пропустить».

`advanceAddDialog()` и `showNextParamAsNewMessage()` используют `nextUnfilledParam()` для определения следующего шага, но различаются способом отображения (редактирование callback-сообщения vs отправка нового).
...
| `periodByLang` | Текст → период | "сегодня", "севодня", "today" → `todayPeriod`; "прошлую неделю", "last week" → `lastWeekPeriod`; "пн", "monday" → `weekdayMondayPeriod` |
...
| week | Понедельник текущей недели 00:00 | Текущий момент |
| lastWeek | Понедельник прошлой недели 00:00 | Понедельник текущей недели 00:00 |
| weekBeforeLast | Понедельник позапрошлой недели 00:00 | Понедельник прошлой недели 00:00 |
| month | 1-е число текущего месяца 00:00 | Текущий момент |
| lastMonth | 1-е число прошлого месяца 00:00 | 1-е число текущего месяца 00:00 |
| monthBeforeLast | 1-е число позапрошлого месяца 00:00 | 1-е число прошлого месяца 00:00 |
| year | 1 января текущего года 00:00 | Текущий момент |
| lastYear | 1 января прошлого года 00:00 | 1 января текущего года 00:00 |
| yearBeforeLast | 1 января позапрошлого года 00:00 | 1 января прошлого года 00:00 |
| weekday (пн–вс) | 00:00 нужного дня | Конец дня (00:00 следующего) или текущий момент, если сегодня |

Логика дней недели: если день уже прошёл на текущей неделе — берётся этот день текущей недели; если ещё не наступил — этот день прошлой недели; если совпадает с сегодня — from = 00:00 сегодня, to = текущий момент.
...
`ListenAndHandle` запускает горутину на каждый апдейт, но ограничивает параллелизм **буферизованным каналом на 100 элементов** (семафор). Дополнительно работает **per-user rate limiter** (`ratelimit.go`): от одного пользователя принимается не более одного запроса в 200мс. Фоновая горутина очищает записи неактивных пользователей.
...
main.go
  ...
  ├─ go a.Run()   -> создание shutdownCtx (для управления жизненным циклом)
  │               -> создание MessageHandler + SessionStore + rateLimiter
  │               -> handler.ListenAndHandle(shutdownCtx)
  │                    └─ Бесконечный цикл: читаем updates -> горутины
  │                       ├─ Per-user rate limit → отсечение частых запросов
  ...
  └─ Ожидание SIGTERM/SIGINT -> a.Shutdown()
                                  ...
                                  ├─ shutdownCancel() → прекращение приёма апдейтов, остановка cleanupLoop
                                  └─ Close DB
```
...
11. **Graceful shutdown через `shutdownCtx`** — `App.Run()` создаёт отдельный `shutdownCtx`, который передаётся в `MessageHandler`, `SessionStore` и `rateLimiter`. При `Shutdown()` контекст отменяется → прекращается приём новых апдейтов и завершаются фоновые горутины. Горутины обработки уже запущенных апдейтов используют `context.Background()`, чтобы in-flight SQL-запросы успели завершиться.
