...
Пользователь нажимает inline-кнопку
    ...
    └───────────┬───────────┘
      ...
      ┌─────┬──┴──┬─────┬─────┬─────┬─────┬─────┬─────┐
      ▼     ▼     ▼     ▼     ▼     ▼     ▼     ▼     ▼
     ex    w/c   d/t    cu    qa    se    sa    sp    x
     │      │     │     │     │     │     │     │     │
     ▼      ▼     ▼     ▼     ▼     ▼     ▼     ▼     ▼
  Выбор  Вес/   Дист/ Ввод  Quick Пок.  Пок.  Выбор Отмена
  упр.   Кол-во Время вручн. Add  упр.  всё   период
...
ReplyKeyboard "Добавить"
  ...
  ┌─────┼──────────┬───────────┐
  ▼     ▼          ▼           ▼
 Reps  RepsWeight  DistTime   Duration
  │     │           │          │
  │    Вес→Кол-во  Дист.→Время Время
  │     │           │          │
  ▼     ▼          ▼           ▼
...
Метод `advanceAddDialog()` проверяет, какие параметры ещё не заполнены для выбранной категории упражнения, и переходит к следующему шагу или сохраняет результат.
